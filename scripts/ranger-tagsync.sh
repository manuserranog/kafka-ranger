...
if [ ! -e ${RANGER_HOME}/.tagsyncSetupDone ]
then
  cd "${RANGER_HOME}"/tagsync || exit
  if ./setup.sh;
  then
    touch "${RANGER_HOME}"/.tagsyncSetupDone
  else
...
RANGER_TAGSYNC_PID=`ps -ef | grep -v grep | grep -i "ranger.tagsync.process.TagSyncProcess" | awk '{print $2}'`
